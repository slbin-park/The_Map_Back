...
const CommunityController = {
    post_community: async (req, res) => {
        try {
            const { home_name, lati, longi, tag, price, site, reason, user_id, category, images } = req.body
            const response = await CommunityService.Save_community(home_name, lati, longi, tag, price, site, reason, user_id, category, images)
            res.json(response);
        } catch (err) {
            console.log(err)
        }
